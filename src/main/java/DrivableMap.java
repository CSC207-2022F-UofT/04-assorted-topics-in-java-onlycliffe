...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String id, Drivable val){
        return drivable_map.putIfAbsent(id, val) == null;
    }
    ...
    public List<Tradable> getTradable() {
        return drivable_map.values().stream()
                .filter(v -> v instanceof Tradable)
                .map(v -> (Tradable) v).collect(Collectors.toList());
    }
